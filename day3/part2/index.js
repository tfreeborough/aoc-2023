...
data.forEach((row, rowIndex) => {
    ...
    characters.forEach((character, columnIndex) => {
        ...
        const isGear = character.match(new RegExp(/\*/));
        if(isSpecial){
            const found = checkAround(rowIndex, columnIndex);
            mapped.push(found);
            if(isGear && found.size === 2){
                let gearRatio = 1;
                for (const value of found) {
                    gearRatio = gearRatio * parseInt(value, 10);
                }
                ratios.push(gearRatio);
            }
        }
    })
})

let sum = ratios.reduce((accumulator, currentValue) => {
